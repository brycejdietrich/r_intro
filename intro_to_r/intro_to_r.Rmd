...
author: Based on material created by Andrew Bray, Mine &Ccedil;etinkaya-Rundel, and
  Mark Hansen and released under a Creative Commons Attribution-ShareAlike 3.0 Unported
  license.
output:
...
The goal of this document is to introduce users to R and RStudio, which we'll be using
...
The best (only?) way to learn about R is to mess around with it, seeing what works. Today we
...
`>` on the console).  You can either type it in manually, copy and paste it
from the pdf document, or open the Rmd file in Rstudio.  If you do the latter, you can immediately send the command to R by placing your cursor on the line and typing Ctrl-Enter.

```{r load-abrbuthnot-data, eval=TRUE}
source("http://www.openintro.org/stat/data/arbuthnot.R")
```

This command instructs R to access a website and fetch some data (detail below).
...
This command will only show the number of boys baptized each year.  Likewise, it is easy to guess how to obtain the number of girls baptized each year.
...
division, then the addition, giving you something that is not a proportion.  R is also case sensitive: if we typed `arbuthnot$Boys` instead of `arbuthnot$boys`, we would get an error.

We could plot the proportion of boys over time (instead of total baptisms per year) by inserting the above command into the previous `plot()` command.  We can make things easy on ourselves by using the up and down arrow keys to scroll through our previous 
commands (so that we can edit the previous plot command).
...
Please create a text file containing R commands that help you answer each question below.  You can also write a sentence or two, if the question requires it.  You do not have to include graphs; just include the commands you used to create the graphs.  Once you have completed the exercises, submit the text file on Blackboard.  If you do not finish in class, please submit it before class on Thursday.
...
1.1.   What years are included in this data set? What are the dimensions of the 
    data frame and what are the variable or column names?

1.2.   How do these counts compare to Arbuthnot's? Are they on a similar scale?

1.3.   Make a plot that displays the boy-to-girl ratio for every year in the data 
...
1.4.   In what year did we see the largest total number of births in the U.S.? You can
